...
<head>
    ...
    <title>แก้ไขข้อมูลประเภทพันธุ์พืช</title>
...
<?php
    ...
    if(isset($_GET['type_id'])){
      ...
      $stmt = $conn->prepare("SELECT* FROM tb_type WHERE type_id=?");
...
<main class="col-md-9 ml-sm-auto col-lg-10 px-md-4 py-4">
            <center><h3>แก้ไขข้อมูลประเภทพันธุ์พืช</h3></center>
            <br><br>
            <nav aria-label="breadcrumb">
                    <ol class="breadcrumb bg-light">
                    <form class="requires-validation" enctype="multipart/form-data" method="post" action="edit_db.php" novalidate>
                    
                    <input type="hidden" name="type_id" required value="<?= $row['type_id'];?>" >
  <div class="form-outline mb-4">
        <label class="form-label" for="form6Example4">ชื่อประเภท</label>
         <input type="text" id="form6Example4" class="form-control" name = "type_name" required value="<?= $row['type_name'];?>"/>  
  </div>
  <div class="form-outline mb-4">
        <label class="form-label" for="form6Example4">รายละเอียด</label>
         <input type="text" id="form6Example4" class="form-control" name = "type_details" required value="<?= $row['type_details'];?>"/>
  </div>
...
&nbsp;&nbsp;&nbsp; &nbsp;
<input type="reset" class="btn btn-danger" value="ยกเลิก" onclick="window.location.href='type.php?val='">
